...
@section('content')
    ...
    <div class="card">
        ...
        <div class="table-responsive text-nowrap px-4 py-2" style="height: 600px;">
            @if (auth()->user()->hasRole(\App\Constants\RoleConst::SUPER_ADMIN))
                <div class="text-end">
                    <a target="_blank" href="{{ route('web.paid-leaves.export') }}" type="button"
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item"
                                            href="{{ route('web.paid-leaves.show', ['id' => $item->uuid]) }}"><i
                                                class="bx bx-show me-1"></i>
                                            Detail</a>
                                        <form id="form_delete_{{ $item->uuid }}"
                                            action="{{ route('web.paid-leaves.delete', ['id' => $item->uuid]) }}"
                                            method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                        </form>
                                        @if (
                                            ...
                                            <a class="dropdown-item" href="javascript:void(0);"
                                                onclick="deleteAction(`{{ $item->uuid }}`, `{{ route('web.paid-leaves.delete', ['id' => $item->uuid]) }}`)"><i
    ...
    @if (auth()->user()->hasRole(\App\Constants\RoleConst::STAFF))
        <div class="buy-now">
            <a href="{{ route('web.paid-leaves.create') }}" class="btn btn-danger btn-buy-now">Tambah
                {{ $module }}</a>
        </div>
    @endif
@endsection
