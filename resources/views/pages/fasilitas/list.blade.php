...
@section('content')
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item"
                                            href="{{ route('web.fasilitas.edit', ['id' => $item->uuid]) }}"><i
                                                class="bx bx-edit-alt me-1"></i>
                                            Edit</a>
                                            <a class="dropdown-item"
                                            href="{{ route('web.fasilitas.show', ['id' => $item->uuid]) }}"><i
                                                class="bx bx-bullseye me-1"></i>
                                            Detail</a>
                                        <form id="form_delete_{{ $item->uuid }}"
                                            action="{{ route('web.fasilitas.delete', ['id' => $item->uuid]) }}"
                                            method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                        </form>
                                        <a class="dropdown-item" href="javascript:void(0);"
                                            onclick="deleteAction(`{{ $item->uuid }}`, `{{ route('web.fasilitas.delete', ['id' => $item->uuid]) }}`)"><i
    ...
    @if (auth()->user()->hasRole(\App\Constants\RoleConst::STAFF) or auth()->user()->hasRole(\App\Constants\RoleConst::SUPER_ADMIN))
    <div class="buy-now">
        <a href="{{ route('web.fasilitas.create') }}" class="btn btn-danger btn-buy-now">Tambah {{ $module }}</a>
    </div>
    @endif
@endsection
@section('script')
